...
class FurnasScraper:
    ...
    def login(self, username, password):
        try:
            ...
            print("1. Fazendo requisição inicial...")
            ...
            print("\n2. Enviando dados de autenticação...")
            ...
            if not response.ok:
                print(f"❌ Erro ao finalizar login: {response.status_code}")
                return False
            
            response_text = response.text
            
            if 'Falha no login' in response_text:
                print("❌ Erro: credenciais inválidas")
                return False
            
            if 'sapLsFPMMessageArea' in response_text:
                print("❌ Erro: mensagem de erro do servidor")
                print(f"Resposta: {response_text[:500]}")
                return False
            
            if 'sapwd_main_window' in response_text and 'Falha no login' not in response_text:
                self.session.cookies.update(response.cookies)
                print("\n✅ Login realizado com sucesso!")
                return True
            
            print("❌ Erro: resposta não esperada do servidor")
            print(f"Resposta: {response_text[:500]}")
            return False
