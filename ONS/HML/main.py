...
def download_latest_bills(session, download_path="C:\\Users\\Bruno\\Downloads\\faturas-ONS", base_url="https://sintegre.ons.org.br"):
    """
    Baixa os PDFs mais recentes dos boletos das faturas da ONS
    """
    try:
        # URLs e caminhos
        download_base = f"{base_url}/sites/1/18/_layouts/15/download.aspx"
        history_url = f"{base_url}/sites/1/18/paginas/servicos/historico-de-produtos.aspx"
        ...
        # Acessa a página de histórico
        print("Acessando página de histórico...")
        response = session.get(history_url, headers=headers)
        soup = BeautifulSoup(response.text, 'html.parser')

        # Encontra todos os links que podem ser PDFs de boletos
        print("Buscando boletos do mês atual...")
        current_month = datetime.now().strftime("%m%Y")
        pdf_links = []
        
        for link in soup.find_all('a', href=True):
            href = link['href']
            if ('Produtos/569' in href and 
                href.endswith('.pdf') and 
                current_month in href):
                source_url = href if href.startswith('/') else f"/{href}"
                download_url = f"{download_base}?SourceUrl={source_url}"
                pdf_links.append(download_url)

        if not pdf_links:
            print("Nenhum boleto encontrado para o mês atual")
            return False

        print(f"Encontrados {len(pdf_links)} boletos para download")

        # Download de cada PDF encontrado
        for download_url in pdf_links:
            try:
                ...
                if response.status_code == 200 and 'application/pdf' in response.headers.get('Content-Type', ''):
                    ...
                    if os.path.exists(filepath) and os.path.getsize(filepath) > 0:
                        ...
                        print(f"✗ Erro: Arquivo vazio ou não salvo - {filename}")
                else:
        ...
        print(f"Erro durante o download dos PDFs: {str(e)}")
        return False
...
def acessar_historico():
    ...
    try:
        ...
        headers = {
            ...
            'Connection': 'keep-alive',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36',
            'Referer': 'https://sintegre.ons.org.br/',
            'Origin': 'https://sintegre.ons.org.br',
            'Host': 'sintegre.ons.org.br',
        ...
        # URL do histórico
        url_historico = "https://sintegre.ons.org.br/sites/1/18/paginas/servicos/historico-de-produtos.aspx"
        
        # Verificar se temos os cookies do Keycloak
        keycloak_cookies = {k: v for k, v in session.cookies.items() if 'KEYCLOAK' in k}
        if not keycloak_cookies:
            print("Cookies do Keycloak não encontrados - Necessário fazer login novamente")
            return False
            
        print("Cookies do Keycloak encontrados:", keycloak_cookies)
        
        # Primeira tentar acessar a página inicial do ONS
        print("Acessando página inicial primeiro...")
        session.get(
        ...
        # Agora tentar acessar o histórico
        max_tentativas = 5
        tempo_espera_base = 30
        for tentativa in range(1, max_tentativas + 1):
            try:
                print(f"Tentativa {tentativa} de {max_tentativas} de acessar a página...")
                
                # Adicionar cookies do Keycloak explicitamente
                for cookie_name, cookie_value in keycloak_cookies.items():
                    session.cookies.set(cookie_name, cookie_value)
                ...
                if response.status_code == 503:
                    tempo_espera = tempo_espera_base * tentativa
                    print(f"Serviço temporariamente indisponível, aguardando {tempo_espera} segundos...")
        ...
        if response.status_code == 503:
            print("Serviço continua indisponível após todas as tentativas")
            return False
        
        if response.status_code != 200:
            print(f"Erro ao acessar página de histórico. Status code: {response.status_code}")
            return False
            
        # Salva o conteúdo para debug
        ...
        print(f"Erro ao acessar histórico: {str(e)}")
        return False
...
def buscar_boletos():
    ...
    try:
        ...
        headers = {
            ...
            'X-RequestDigest': obter_request_digest(),
            'Accept': 'application/json'
        }
        
        # Fazer requisição para API que retorna os boletos
        data = {
        ...
        response = session.post(
            'https://sintegre.ons.org.br/sites/1/18/_api/web/lists/getbytitle(\'Produtos\')/items',
            json=data,
            headers=headers
        )
        
        if response.status_code != 200:
            print(f"Erro ao buscar boletos. Status: {response.status_code}")
            return []
            
        return response.json()['value']
        
    except Exception as e:
        print(f"Erro ao buscar boletos: {str(e)}")
        return []
...
if __name__ == "__main__":
    try:
        ...
        # Cria uma sessão com os cookies
        session = requests.Session()
        for cookie_name, cookie_value in auth_cookies.items():
            session.cookies.set(cookie_name, cookie_value)
            
        # Define o caminho para salvar os arquivos
        download_path = r"C:\Users\Bruno\Downloads\faturas-ONS"
        
        # Faz o download dos PDFs
        success = download_latest_bills(session, download_path)
        if success:
            print("Download concluído com sucesso!")
        else:
            print("Houve um erro durante o download")
            
        # Tentar acessar o histórico
        if acessar_historico():
            boletos = buscar_boletos()
            if boletos:
                print(f"Encontrados {len(boletos)} boletos")
            else:
                print("Nenhum boleto encontrado para o mês atual")
        else:
            print("Não foi possível acessar a página de histórico")
