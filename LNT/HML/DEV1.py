...
class SigetPlusDownloader:
    ...
    def __init__(self, download_dir=None, wkhtmltopdf_path=None):
        ...
        if download_dir:
            ...
            self.download_dir = os.path.join(os.path.expanduser("~"), "Downloads", "TUST", "LNT")
...
if __name__ == "__main__":
    print("=" * 50)
    print("SIGET PLUS - Download de Faturas da LNT")
    print("=" * 50)
    print("Este script baixa faturas do site da Siget Plus para a Luziania-Niquelandia Transmissora")
    print("As faturas serão organizadas em pastas separadas para cada empresa")
    print(f"Diretório base: C:\\Users\\Bruno\\Downloads\\TUST\\LNT")
    print("=" * 50)
    ...
    download_dir = "C:\\Users\\Bruno\\Downloads\\TUST\\LNT"
    ...
    empresa_opcao = input("\nDigite o código da empresa que deseja baixar (padrão: 3748): ") or "3748"
    
    # Definir agentes a serem processados
    if empresa_opcao in TRANSMISSORAS:
        # Empresa específica
        agente = empresa_opcao
        print(f"\nBaixando faturas para a empresa {TRANSMISSORAS[empresa_opcao]} (código {empresa_opcao}).")
    else:
        print(f"Código de empresa inválido: {empresa_opcao}. Usando o padrão (3748).")
        agente = "3748"
    ...
    if opcao == "1":
        ...
        # Processar o agente
        print(f"\nIniciando download para o agente {agente} ({TRANSMISSORAS.get(agente, 'Desconhecido')}) no período {periodo}...")
        faturas_baixadas = downloader.baixar_faturas_periodo(agente, periodo)
        print(f"Download concluído para o agente {agente}. Total de faturas baixadas: {faturas_baixadas}")
        ...
        # Processar o agente
        print(f"\nProcessando agente: {agente} ({TRANSMISSORAS.get(agente, 'Desconhecido')})")
        resultados = downloader.baixar_faturas_multiplos_periodos(agente, periodos)
