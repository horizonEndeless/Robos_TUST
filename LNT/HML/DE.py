...
if __name__ == "__main__":
    ...
    # Obter todas as empresas disponíveis
    agentes = list(TRANSMISSORAS.keys())
    print(f"\nBaixando faturas para todas as empresas: {len(agentes)} empresa(s)")
    for codigo, nome in TRANSMISSORAS.items():
        print(f"- {codigo}: {nome}")
    
    # Obter o mês anterior ao atual para download
    data_atual = datetime.now()
    # Calcular mês anterior
    mes_anterior = data_atual.month - 1
    ano_anterior = data_atual.year
    if mes_anterior == 0:  # Se for janeiro, voltar para dezembro do ano anterior
        mes_anterior = 12
        ano_anterior = data_atual.year - 1
    
    periodo_anterior = f"{ano_anterior}{mes_anterior:02d}"
    print(f"\nBaixando faturas do mês anterior: {periodo_anterior}")
    ...
    for agente in agentes:
        ...
        faturas_baixadas = downloader.baixar_faturas_periodo(agente, periodo_anterior)
        resultados[agente] = faturas_baixadas
    ...
    for agente, total in resultados.items():
        nome_agente = TRANSMISSORAS.get(agente, agente)
        print(f"Agente {agente} ({nome_agente}): {total} faturas")
    print("-" * 50)
    ...
    print("\nDownload concluído. Pressione Enter para sair...")
    input()