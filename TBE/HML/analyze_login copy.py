...
def baixar_xml(session, xml_link, download_dir, info):
    ...
    try:
        ...
        if not xml_link.startswith('http'):
            xml_link = f"https://portalcliente.tbenergia.com.br{xml_link}"
        
        logger.info(f"Baixando XML de {xml_link}...")
        
        # Fazer a requisição para baixar o XML
        response = session.get(xml_link)
